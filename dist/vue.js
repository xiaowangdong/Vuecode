...
(function (global, factory) {
  ...
  function initRender (vm) {
    ...
    vm.$createElement = function (a, b, c, d) { return createElement(vm, a, b, c, d, true); }; // 关于vdom的创建
  ...
  function initEvents (vm) {
    ...
    var listeners = vm.$options._parentListeners; // 拿出父组件中注册的一些事件监听器
    if (listeners) {
  ...
  function mountComponent ( // 执行挂载  Execute mount
    vm,
    el,
    hydrating
  ) {
    ...
    var updateComponent; // 更新组件
    /* istanbul ignore if */
    if ( config.performance && mark) {
      updateComponent = function () { // 更新组件此时仅做了声明,并没有进行调用,调用是在new Watcher实例创建之后,若以后有更新Wathcer就会让更新组件再次执行
        var name = vm._name;
        var id = vm._uid;
      ...
      updateComponent = function () { // 更新组件此时仅做了声明,并没有进行调用,调用是在new Watcher实例创建之后,若以后有更新Wathcer就会让更新组件再次执行  updateComponent is now declared and not invoked. It is invoked after the Watcher instance is created, and later updates to Watcher will cause updateComponent to execute again
  ...
  function initUse (Vue) {
    Vue.use = function (plugin) { // Vue.use() 会将插件名传入 Introduced to the plugin
